...
def MyKmeans(hg, gamma=1):
    ...
    vec_real = vec.real  # 返回的数值为负数，其虚部都为0，直接取实数部分
    kk = KMeans(n_clusters=optimal_k)
    y_pred = kk.fit_predict(vec_real)

    # 获取Inertia值
    inertia = kk.inertia_
    # 计算轮廓系数
    silhouette = metrics.silhouette_score(vec_real, y_pred)
    # CH指标
    calinski_harabasz = metrics.calinski_harabasz_score(vec_real, y_pred)
    ...
    return ok_group,[inertia,silhouette,calinski_harabasz]
...
def GetIntention(hg, k, y_pred,argminC, intention_edge_support=0.7):
    ...
    for cluster in clusters:
        ...
        # 被意图超边覆盖70%的点即为正样本
        # pos_id = list(np.where((new_arr != 0).sum(axis=1) / new_arr.shape[1] > intention_edge_support)[0])
...
def RunScene(SamplesPath,min_support = 0.3,mu = 10,gamma = 0.8):
    ...
    for i in range(l-1,-1,-1):
        for j,oneset in enumerate(edges_set[i]):
            ...
            start_time = time.time()
            ok_group[(i, j)],kmeans_score[(i, j)]= MyKmeans(hg, gamma=gamma)
            end_time = time.time()
    ...
    return jac,LIP,MLMSSM,best_argminC,best_avgconf,kmeans_score[best_edge_id]
...
if __name__ == '__main__':
    ...
    Num = [10, 20, 40, 60, 80, 100]  # 样本数量 10, 20, 40, 60, 80, 100
    ...
    for Scene in Scene_List:
        ...
        Score_df = pd.DataFrame(
            columns=['Scene', 'Sindex', 'Num', 'Noise_Rate', 'Jaccard', 'LIP', 'MLMSSM', 'best_argminC',
                     'best_avgConf','inertia','silhouette','calinski_harabasz'])
        df_id = 0
            ...
            for num in Num:
                for p in P:
                    SamplesPath = Scene_index_Folder + "\\num" + str(num) + "\\p" + str(p) + "_" + str(
                        repeat_index) + ".json"
                    jac, LIP, MLMSSM, best_argminC, best_avgconf,kmeans_score = RunScene(SamplesPath, min_support, mu,
                                                                                  gamma)
                    Score_df.loc[df_id] = [Scene, folders[i], num, p, jac, LIP, MLMSSM, best_argminC,
                                           best_avgconf] + kmeans_score
                    df_id = df_id + 1

        out_file_path = 'results\\20240409_score_gamma07_numall_Rp2.csv'
