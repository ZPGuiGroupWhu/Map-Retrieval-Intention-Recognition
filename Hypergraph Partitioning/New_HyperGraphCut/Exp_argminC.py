...
def RunScene(SamplesPath,min_support = 0.3,mu = 10,gamma = 0.8):
    ...
    writer_process.write_rows([['jac','LIP','MLMSSM','avgConf','cc']])
    for edges_id, k_groups in ok_group.items():
        for k, values in k_groups.items():

            # 计算每一组的精度评价指标，证明argminCC有效
            jac = CalJaccard(TintVid, values['PintV'])
            Precision, Recall = CalRec_Pre(TrueInts, values['PredInts'], mu)
            LIP = 1 - Precision
            MLMSSM = Recall
            # print("(i,j)=",edges_id,",k=",k,":jac=%.3f" %jac,",Precision=%.3f"%Precision,",Recall=%.3f"%Recall,"——avgconf=%.3f"%values['avgconf'],",argminC=%.3f"%values['argminC'])
            if values['argminC']<=1:
                writer_process.write_rows([[jac, LIP,  MLMSSM, values['avgconf'], values['argminC']]])
...
out_file_path = 'results\\20240403_argminCC_gamma07_num20_Rp2.csv'
# 打开 CSV 文件并创建 csv.writer 对象
if not os.path.exists(os.path.dirname(out_file_path)):
    os.makedirs(os.path.dirname(out_file_path))

# 使用
writer_process = CSVWriter(out_file_path)  # 获取全局的 CSV 文件写入指针实例

# 打开文件并写入数据行
writer_process.open_file(out_file_path)
...
if __name__ == '__main__':
    ...
    writer_process.write_rows([['mu', mu,'gamma',gamma,'min_support',min_support]])
    ...
    for Scene in Scene_List:
        ...
        writer_process.write_rows([['Scene', 'Sindex', 'Num', 'Noise_Rate', 'Jaccard', 'LIP', 'MLMSSM', 'best_argminC',
                     'best_avgConf']])
            ...
            for num in Num:
                for p in P:
                    ...
                    jac, LIP, MLMSSM, best_argminC, best_avgconf = RunScene(SamplesPath, min_support, mu,
                                                                                  gamma)
                    writer_process.write_rows(
                        [[Scene, folders[i], num, p, jac, LIP, MLMSSM, best_argminC,
                                           best_avgconf]])
