...
def CalMultiLableDis(intention1, intention2):
    # 补全长度不一致
    dimlen = {}
    for dim in DimList:
...
def generate_new_combinations(freq_set, k):
    ...
    for i in range(sets_len):
        for j in range(i + 1, sets_len):
            ...
            new_content = list(set(freq_item) - set(freq_set_list[i]))
            if len(freq_item) == k and len(new_content) == 1:
                s = new_content
                for dim in DimList:
                    if new_content[0] in AllIntent[dim]:
                        s += Tree[dim]["all_ancestors"][new_content[0]] + Tree[dim]["all_hyponyms"][new_content[0]] + \
                             Tree[dim]["equivalent_relations"][new_content[0]]
                        s = list(set(s))
                        break

                flag = 1
                for content in freq_set_list[i]:
                    if content in s:
                        flag = 0
                        break

                if flag: new_combinations.add(freq_item)

    return new_combinations


# 循环生成候选集频繁集
def AprioriEdges(data_set, min_support, max_len=None):
...
# 频繁项集生成候选超边
# 取频繁二项集
def EdgesSet(Samples, min_support=0.3):
    # 上位节点集合
    AncesU = {}
    for dim in DimList:
        AncesU[dim] = []
    ...
    L, support_data = AprioriEdges(data, min_support=min_support,max_len = 2)
    ...
    for li in L2:
        ...
        # 相互有关联的在一个onegroup
        for id, onecombinations in enumerate(li):
            # 组合中不可有任意一个不可合并
        ...
        for i in combi:
            ...
            if k not in alledges and k not in cmtcombi:
                cmtcombi.append(k)

        if cmtcombi:
            alledges += cmtcombi
            edges_set.append(cmtcombi)
...
class SampleGraph(object):
    def __init__(self, Samples, wIndex=1):
        ...
        # 邻接矩阵 A = long(V) × long(V)
        self.A, self.Dv = self.CreateA()
    ...
    def CreateA(self, reset_flag=0):
        A = np.zeros([len(self.V), len(self.V)])  # v * e
        Dv = np.zeros([len(self.V), len(self.V)])  # e * e

        for i in range(len(self.V)):
            for j in range(i + 1, len(self.V)):
                # 计算语义相似度，这里假设相同属性的点之间的相似度为1，不同属性的点之间的相似度为0
                dis = CalMultiLableDis(self.V[i], self.V[j])
                similarity = math.exp(-dis * dis / mu)
                if similarity > 0:
                    A[i][j] = A[j][i] = similarity

        # 统计每行非零值的个数
        non_zero_counts = np.count_nonzero(A, axis=1)
        for index, value in enumerate(non_zero_counts):
            Dv[index][index] = value
...
def GetIntention(g, k, y_pred,intention_edge_support=0.7):
    # 类簇
    clusters = []
    # 意图
    PredInts = []
    ...
    for i in range(k):
        clusters.append({})
        clusters[i]['V'] = []
        clusters[i]['S'] = [] # 簇中的顶点id
        clusters[i]['Conf'] = 0

    # 聚类簇顶点分组
    for vIndex, v in enumerate(g.V):
        if y_pred[vIndex] >= 0:
    ...
    for cluster in clusters:
        ...
        intention_edge_set = SampleEdgesSet(Samples=cluster['V'], min_support=intention_edge_support)
        # 频繁1项集下的边的组合
        PredInts.append(intention_edge_set[0][0])


    PintV = getPintV(PredInts, g.V)
    PintV = list(set(PintV))

    V_id = np.arange(g.A.shape[0])
...
def RunScene(SamplesPath,mu = 10):
    ...
    # 简单图
    # 定义样本点和它们之间的关系
    n = len( Samples)  # 样本点的数量

    # 实例化图
    g = SampleGraph(Samples=Samples, wIndex=mu)

    I = np.eye(len(Samples))
    # 节点的度不能为0！！！！构图前要检查
    L = I - fractional_matrix_power(g.Dv, -0.5) @ g.A @ fractional_matrix_power(g.Dv, -0.5)

    # 再次执行k-means算法得到最终的聚类结果
    optimal_k = len(TrueInts)
    if optimal_k == 1:

        one_y_pred = [0] * len(Samples)

        clusters, PredInts, PintV, PnoiseV = GetIntention(g, optimal_k, one_y_pred)
        ...
        clusters, PredInts, PintV, PnoiseV = GetIntention(g, optimal_k, y_pred)

    jac = CalJaccard(TintVid, PintV)
    Precision, Recall = CalRec_Pre(TrueInts, PredInts, mu)
    LIP = 1-Precision
    MLMSSM = Recall
            ...
            for num in Num:
                for p in P:
                    SamplesPath = Scene_index_Folder + "\\num" + str(num) + "\\p" + str(p) + "_" + str(
                        repeat_index) + ".json"
                    jac, LIP, MLMSSM = RunScene(SamplesPath, mu)

                    Score_df.loc[df_id] = [Scene, folders[i], num, p, jac, LIP, MLMSSM]
                    df_id = df_id + 1

        out_file_path = 'results\\20240403_SampleGraph_num20_Rp2.csv'
