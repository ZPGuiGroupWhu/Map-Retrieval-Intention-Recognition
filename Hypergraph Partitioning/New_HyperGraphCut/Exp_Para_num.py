...
def RunScene(SamplesPath,min_support = 0.3,mu = 10,gamma = 0.8):
    ...
    return jac,LIP,MLMSSM,best_argminC,best_avgconf,Graph_time,Cluster_time
    ...
    for Scene in Scene_List:
        ...
        Score_df = pd.DataFrame(
            columns=['Scene', 'Sindex', 'Num', 'Noise_Rate', 'Jaccard', 'LIP', 'MLMSSM', 'best_argminC',
                     'best_avgConf','Graph_time','Cluster_time'])
        df_id = 0
            ...
            for p in P:
                for num in Num:
                    SamplesPath = Scene_index_Folder + "\\num" + str(num) + "\\p" + str(p) + "_" + str(
                        repeat_index) + ".json"
                    jac, LIP, MLMSSM, best_argminC, best_avgconf,Graph_time,Cluster_time = RunScene(SamplesPath, min_support, mu,
                                                                                  gamma)
                    Score_df.loc[df_id] = [Scene, folders[i], num, p, jac, LIP, MLMSSM, best_argminC,
                                           best_avgconf,Graph_time,Cluster_time]
                    df_id = df_id + 1

        out_file_path = 'results\\20240410_par_num_gamma07_time_Rp2.csv'
