...
def MyKmeans(hg,optimal_k, gamma=1):
    ...
    # 外部输入子意图数量k，执行k-means算法得到最终的聚类结果

    if optimal_k == 1:
        # 取k=2时的 argminC
        val, vec = topvec(L, 2)
        vec_real = vec.real  # 返回的数值为负数，其虚部都为0，直接取实数部分
        y_pred = KMeans(n_clusters=2).fit_predict(vec_real)
        argminC = CalargminC(hg, y_pred)
        argminC = 0 - argminC
        one_y_pred = [0] * len(hg.V)

        clusters, PredInts, PintV, PnoiseV = GetIntention(hg, optimal_k, one_y_pred, argminC)
...
def RunScene(SamplesPath,min_support = 0.3,mu = 10,gamma = 0.8):
    ...
    for i in range(l-1,-1,-1):
        for j,oneset in enumerate(edges_set[i]):
            ...
            start_time = time.time()
            k = len(TrueInts)
            ok_group[(i, j)] = MyKmeans(hg,k,gamma=gamma)
            end_time = time.time()
...
if __name__ == '__main__':
    ...
    for Scene in Scene_List:
        ...
        out_file_path = 'results\\20240403_argminC_k_gamma07_num20_Rp2.csv'
