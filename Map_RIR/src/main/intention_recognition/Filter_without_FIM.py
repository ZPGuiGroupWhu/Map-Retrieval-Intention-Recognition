# Mining  intention candidates without FIM
        ...
        for sub_intention_b in sub_intentions[:]:
            ...
            if sub_intention_a1 != sub_intention_b1 and sub_intention_a1[0][0] == sub_intention_b1[0][0]:
                ...
                if (sub_intention_a_covered_relevance_samples == sub_intention_b_covered_relevance_samples) \
                        and RetrievalUtil.is_inclusive(sub_intention_a1[0], sub_intention_b1[0], data.Ancestor,
                                                       data.Ontology_Root):
                    # print("sub_intention_a", sub_intention_a)
                    # print("sub_intention_b", sub_intention_b)
                    sub_intentions.remove(sub_intention_a)
                    break
...
def get_sub_intentions(data, concepts, min_support, k_max):
    ...
    for tmp_S in concepts['Spatial']:
        for tmp_F in concepts['MapContent']:
            for tmp_M in concepts['MapMethod']:
                for tmp_T in concepts['Theme']:
                    tmp_sub_intention = frozenset(
                        {('Spatial', tmp_S), ('MapContent', tmp_F), ('MapMethod', tmp_M), ('Theme', tmp_T)})
                    tmp_sub_intention_covered_relevance_samples = \
                        get_sub_intention_covered_samples(tmp_sub_intention, all_concepts_covered_samples,
                                                          "relevance")
                    tmp_sub_intention_covered_relevance_samples_num = len(tmp_sub_intention_covered_relevance_samples)
                    tmp_sub_intention_support = float(
                        tmp_sub_intention_covered_relevance_samples_num) / len(samples["relevance"])
                    if tmp_sub_intention_support > min_support:
                        result.append(tmp_sub_intention)
    result1 = result
    for sub_intention_a in result:
        for sub_intention_b in result:
            sub_intention_a1 = list(sub_intention_a)
            sub_intention_b1 = list(sub_intention_b)
            if sub_intention_a1 != sub_intention_b1:
                sub_intention_a_covered_relevance_samples = \
                    get_sub_intention_covered_samples(sub_intention_a1, data.all_relevance_concepts_retrieved_docs,
                                                      "relevance")
                sub_intention_b_covered_relevance_samples = \
                    get_sub_intention_covered_samples(sub_intention_b1, data.all_relevance_concepts_retrieved_docs,
                                                      "relevance")
                if sub_intention_a_covered_relevance_samples == sub_intention_b_covered_relevance_samples:
                    if MDL_RM.is_intention_cover(transform_sub_intention(sub_intention_b, data.Ontology_Root),
                                                     transform_sub_intention(sub_intention_a, data.Ontology_Root),
                                                     data.Ancestor,
                                                     data.Ontology_Root):
                        result1.remove(sub_intention_b)

    result = [transform_sub_intention(x, data.Ontology_Root) for x in result1]
    return result
            ...
            for tmp_M in concepts['MapMethod']:
                for tmp_T in concepts['Theme']:
                    ...
                    tmp_sub_intention_covered_relevance_samples = \
                        get_sub_intention_covered_samples(tmp_sub_intention, all_concepts_covered_samples,
                                                          "relevance")
                    tmp_sub_intention_covered_irrelevance_samples_num = len(
                        tmp_sub_intention_covered_irrelevance_samples)
                    tmp_sub_intention_covered_relevance_samples_num = len(tmp_sub_intention_covered_relevance_samples)
                    tmp_sub_intention_support = float(
                        tmp_sub_intention_covered_irrelevance_samples_num) / len(samples["irrelevance"])
                    not_support = float(tmp_sub_intention_covered_relevance_samples_num) / len(samples["relevance"])
                    if tmp_sub_intention_support > min_support :
                        if tmp_sub_intention_support > not_support:
                            result.append(tmp_sub_intention)
