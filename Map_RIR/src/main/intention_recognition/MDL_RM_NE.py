...
from Map_RIR.src.main.util.RetrievalUtil import retrieve_docs_based_on_terms_covered_samples, is_negative_paste, \
    retrieve_docs_based_on_terms_covered_samples1
from Map_RIR.src.main.intention_recognition import Config
...
def get_model_encoding_length(intention_covered_sample_nums):
    intention_num = len(intention_covered_sample_nums)  # 子意图个数
...
def get_rules_encoding_length_without_order_constraint(rules, positive_samples, negative_samples,
    ...
    new_rules_list = []
    all_negative_sub_intention_num = 0
    for tmp_rule in rules:
        ...
        # 判断负标签的贴入
        negative_sub_intention_num = 0
        if negative_rules != {}:
            for tmp_negative_intention in negative_rules:
                paste_dim = is_negative_paste(tmp_rule[0], tmp_negative_intention, Data.Ancestor, Data.Ontology_Root)
                if paste_dim == 0:
                    continue
                else:
                    negative_sub_intention_num = negative_sub_intention_num + 1
                    # if negative_sub_intention_num != 0:
                    #     print(tmp_rule[0])
                    # 负标签的贴入,子意图长度变化
        tmp_intention_encoding_length += \
            log2(tmp_rule_covered_samples_num) + get_sub_intention_encoding_length() * (1 + negative_sub_intention_num)
        all_negative_sub_intention_num += negative_sub_intention_num
...
def get_merge_pair_statistics_without_order_constraint(data,merge_pair, rules, positive_samples, negative_samples,
    ...
    # 样本检索优化后
    tmp_merged_rule_covered_positive_samples_id = \
        retrieve_docs_based_on_terms_covered_samples1(data,tmp_merged_rule, data.all_relevance_concepts_retrieved_docs,
                                                      "positive", negative_rules)
    tmp_merged_rule_covered_negative_samples_id = \
        retrieve_docs_based_on_terms_covered_samples1(data,tmp_merged_rule, data.all_relevance_concepts_retrieved_docs,
    ...
    for tmp_rule_id in rules:
        ...
        if is_intention_cover(tmp_merged_rule, tmp_rule_content, Data.Ancestor, Data.Ontology_Root):  # 意图包含另一个意图？
            continue
...
def get_intention_by_method6(samples, data_encoding_method, random_merge_number,
    ...
    return best_rules.rules, best_rules.total_encoding_length, init_min_encoding_length, method_log, negative_rules
