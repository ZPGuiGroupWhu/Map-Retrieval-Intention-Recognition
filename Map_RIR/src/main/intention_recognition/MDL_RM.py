...
from Map_RIR.src.main.util.RetrievalUtil import retrieve_docs_based_on_terms_covered_samples
from Map_RIR.src.main.intention_recognition import Config

# print(sys.path)
sub_negative_encoding_length = None
...
def get_sub_negative_encoding_length():
    global sub_negative_encoding_length
    if sub_negative_encoding_length is not None:
        return sub_negative_encoding_length
    else:
        sub_intention_num = 0
        for tmp_dim in Data.dimensions:
            #     print(len(Data.Ontologies[tmp_dim]))
            sub_intention_num += len(Data.Ontologies[tmp_dim])
        sub_negative_encoding_length = log2(sub_intention_num)
        return sub_negative_encoding_length
