# coding:utf-8
# Searching with the greedy algorithm
import time
import random
...
def get_intention(samples, candidate_sub_intentions_list, random_merge_number,
    ...
    while True:
        ...
        tempnum_min_rules = tmp_rules
        # print("temp_length",tmp_rules.total_encoding_length)
        # for subIntention in tmp_rules.intention:
            # print("P：", subIntention.positive_sub_Intention)
            # print("N：", subIntention.negative_sub_Intention)
        for temp_candidate_sub_intention in candidate_sub_intentions_list:
            temp_intent = copy.deepcopy(tmp_rules.intention)
            # print("tmp_rules.intention",len(tmp_rules.intention))
            temp_intent.append(temp_candidate_sub_intention)
            ...
            temp_intent_uncovered_positive_samples_id = tmp_merged_rule_statistics[-4]
            # remain_uncovered_negative_samples_id
            temp_intent_uncovered_negative_samples_id = tmp_merged_rule_statistics[-3]
            ...
            # 编码长度不变，意图更新
            isRenew2 = False
            new_rules1 = Rules(temp_intent_uncovered_positive_samples_id,
                               temp_intent_uncovered_negative_samples_id,
                               temp_intent, tmp_encoding_length)
            if tmp_total_encoding_length < tempnum_min_rules.total_encoding_length:
                isRenew = True

                tempnum_min_rules = new_rules1
            if tmp_total_encoding_length == tempnum_min_rules.total_encoding_length:
                for temp_sub_intent in tempnum_min_rules.intention:
                    for new_sub_intent in temp_intent:
                        # 判断包含关系
            ...
            if Config.TAG_RECORD_PROCESS:
                ...
                tmp_rules_log = {
                    ...
                    "total_encoding_length": new_rules1.total_encoding_length,
                    "intention_encoding_length": new_rules1.intention_encoding_length,
                    "sample_encoding_length": new_rules1.sample_encoding_length,
                    "encoding_length_compression_rates": new_rules1.total_encoding_length / init_min_encoding_length,
                    "intention_result": new_rules1.intention
                }
                if isRenew2:
                    process_log.pop()

                process_log.append(tmp_rules_log)
        if tempnum_min_rules == tmp_rules:
            # print("最短：", tmp_rules.total_encoding_length, tmp_rules.intention_encoding_length,
            #      tmp_rules.sample_encoding_length)
            time07 = time.time()
            # print("time_use_calculate_merge_statistic",time_use_calculate_merge_statistic)
            return tmp_rules, time07 - time00, process_log
        tmp_rules = tempnum_min_rules
...
if __name__ == "__main__":
    ...
    test_sample_path = "./../../../resources/samples/" + sample_version + "/Scene" + scene + "/noise_samples_S0p3_L0p8.json"
