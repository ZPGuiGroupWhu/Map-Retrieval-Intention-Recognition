# Intention recognition incorporating positive and negative preferences
import time

import random
from math import log2, gamma
from Map_RIR.src.main.intention_recognition import FrequentItemsets, MDL_RM
import copy

from Map_RIR.src.main.intention_recognition.Config import With_negative
...
def get_intention(samples, candidate_sub_intentions_list, random_merge_number,
    ...
    time00 = time.time()
    candidate_num = len(candidate_sub_intentions_list)
    # print("candidate_sub_intentions_num", candidate_num)
    # random_merge_number is a multiple of candidate_num
    random_merge_number = candidate_num * random_merge_number
    #固定迭代次数为1000
    #random_merge_number = 500
    # 初始化
    ...
    while True:
        ...
        if random_merge_number <= 0 or not candidate_sub_intentions_list:
            ...
            return tmp_rules,time07-time00
        # rules_num：规则的最大数量
    ...
    if With_negative == True:
        ...
        candidate_items = {
            ...
            'irrelevance': FrequentItemsets.get_all_irrelevance_candidate_sub_intentions(samples,
                                                                                         min_support)}
        time22 = time.time()
        # print("get_all_relevance_and_irrelevance_candidate_sub_intentions_time:", time22-time11)
        ...
        time33 = time.time()
        # print("relevance_and_irrelevance_candidate_sub_intentions_merge_time:", time33-time22)
    if With_negative == False:
        sub_relevance_intentions = FrequentItemsets.get_all_relevance_candidate_sub_intentions(samples, min_support)
        for sub_relevance_intention in sub_relevance_intentions:
            candidate_sub_intentions_list.append(SubIntention(sub_relevance_intention,[]))
...
def run_IM(samples, min_support, random_merge_number):
    ...
    result, time_all = get_intention(samples, candidate_sub_intentions_list, random_merge_number, 2, 0.3)
...
if __name__ == "__main__":
    ...
    sample_version = "scenes_v5/scenes_v5"
    ...
    result, time_IM = run_IM(samples, min_support, 10)
    time2 = time.time()
    print("timeall",time2-time1)
    method_result1 = MDL_RM \
        .get_intention_by_method6(samples, data_encoding_method, 50, 2, 0.3)
