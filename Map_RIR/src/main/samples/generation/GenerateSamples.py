...
intention_path = "../../../../resources/samples/Intention7.30.json"
intention_csv_path = "../../../../resources/samples/各场景具体意图7.30.csv"
...
def get_intention_v2_file():
    content_concept = {"TransitionMetal": "http://sweetontology.net/matrElement/TransitionMetal",
                       ...
                       "InternalWave": "http://sweetontology.net/phenWave/InternalWave",
                       "Insect": "http://sweetontology.net/matrAnimal/Insect",
                       "PotentialTemperature": "http://sweetontology.net/propTemperature/PotentialTemperature",
                       "MesoscaleWind": "http://sweetontology.net/phenAtmoWindMesoscale/MesoscaleWind",
                       "ElectronTemperature": "http://sweetontology.net/propTemperature/ElectronTemperature",
                       "LocalWind": "http://sweetontology.net/phenAtmoWind/LocalWind",
                       "MarineAnimal": "http://sweetontology.net/matrAnimal/MarineAnimal",
                       "IgneousRock" : "http://sweetontology.net/matrRockIgneous/IgneousRock",
                       "None": "None"}
    intention_v2_csv = load_csv(intention_csv_path)
    tmp_intent = []
    tmp_sub_intention = {"positive": {}, "negative": []}
    tmp_intention_name = None
    intentions = {}
    for tmp_line in intention_v2_csv:
        if len(tmp_line[0]) > 0:
            # 如果已经开始下一个意图，则将意图放入意图哈希表
            tmp_intent.append(tmp_sub_intention)
            intentions[copy.deepcopy(tmp_intention_name)] = copy.deepcopy(tmp_intent)
            tmp_intent = []
            #tmp_intention_name = tmp_line[0].replace("场景", "Scene")
            tmp_intention_name = tmp_line[0]
            tmp_sub_intention = {
                "positive": {"T": tmp_line[2], "C": content_concept[tmp_line[3]], "S": tmp_line[4], "M": tmp_line[5]},
                "negative": []}
        else:
            # 当前意图还有其他的子意图(或负意图)
            if len(tmp_line[1]) <= 0:
                ...
                tmp_intent.append(tmp_sub_intention)
                tmp_sub_intention = {
                    "positive": {"T": tmp_line[2], "C": content_concept[tmp_line[3]], "S": tmp_line[4],
                                 "M": tmp_line[5]},
                    "negative": []}
            else:
                # 正向
                ...
                for dim in range(2, 5):
                    if tmp_line[dim] != "None":
                        if dim == 3:
                            tmp_sub_intention["negative"].append({"C": content_concept[tmp_line[3]]})
                        else:
                            tmp_sub_intention["negative"].append({dims[dim-2]: tmp_line[dim]})
    tmp_intent.append(tmp_sub_intention)
