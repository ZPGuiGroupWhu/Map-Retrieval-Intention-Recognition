...
from Map_RIR.src.main.util import RetrievalUtil
...
class Data:
    ...
    def __init__(self, relevance_feedback_samples, real_intention=None):
        ...
        self.dimensions = list(relevance_feedback_samples["relevance"][0].keys())

        self.all_relevance_concepts = {}    # 所有正样本中出现的概念及其所有祖先概念
        self.all_irrelevance_concepts = {}   # 所有负样本中出现的概念及其所有祖先概念
        for dim in self.dimensions:
            self.all_relevance_concepts[dim] = self.get_all_relevance_concepts(dim)
            self.all_irrelevance_concepts[dim] = self.get_all_irrelevance_concepts(dim)
        ...
        for tmp_dim in self.all_relevance_concepts.keys():
            tmp_dim_all_relevance_concepts = self.all_relevance_concepts[tmp_dim]+self.all_irrelevance_concepts[tmp_dim]
    ...
    # 获得某个维度在相关文档中出现的概念
    def get_doc_relevance_concepts(self, dim_name):
        # 只有正样本中的概念
        relevance_docs = self.docs["relevance"]

        # relevance_docs = self.docs["relevance"]+self.docs["irrelevance"]
        result = set()
    ...
    def get_doc_irrelevance_concepts(self, dim_name):
        # 只有正样本中的概念
        relevance_docs = self.docs["irrelevance"]
    ...
    def get_all_relevance_concepts(self, dim_name):
        result = self.get_doc_relevance_concepts(dim_name)
    ...
    def get_all_irrelevance_concepts(self, dim_name):
        result = self.get_doc_irrelevance_concepts(dim_name)
        # 如果当前维度取值为枚举类型，则不存在上位概念，此维度的相关概念只包括相关文档中出现的概念
        if Data.Ancestor[dim_name] is None:
            return result
